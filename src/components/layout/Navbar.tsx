...
function MegaMenu({ menu, open, onOpen, onClose }: { menu: Menu; open: boolean; onOpen: () => void; onClose: () => void }) {
  ...
  return (
    <div
      ...
      className="anim-pop absolute left-1/2 top-full z-50 w-[44rem] -translate-x-1/2 pt-4"
    >
...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6">
      ...
      {open && (
        <div className="anim-drop container-x mt-3 lg:hidden">
                      ...
                      <Collapse open={expanded}>
                        <div className="mb-2 ml-4 flex flex-col border-l border-line pl-2">
                          {menu.items.map((sub) => (
                            ...
                            >
                              <sub.icon className="h-4 w-4 text-muted" strokeWidth={1.75} />
                              {sub.title}
                            </Link>
                          ))}
                        </div>
