...
import { industryIcons as icons } from "@/components/industries/industryIcons";
            ...
            return (
              <Reveal key={ind.slug} delay={i * 0.04}>
                <Link
                  ...
                  className="group flex h-full flex-col rounded-3xl border border-line-dark p-6 transition-colors hover:border-cream/40 hover:bg-cream/[0.04]"
                >
                  <div className="flex items-center justify-between">
                    <Icon className="h-6 w-6" strokeWidth={1.75} />
                    <ArrowUpRight className="h-4 w-4 text-cream/50 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                  </div>
                  <h3 className="mt-8 font-display text-lg font-semibold tracking-tight">{ind.title}</h3>
